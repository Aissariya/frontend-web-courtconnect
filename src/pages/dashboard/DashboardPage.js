...
const DashboardPage = () => {
  ...
  const [revenueChartFilterPeriod, setRevenueChartFilterPeriod] =
    useState("daily");
  const [pieChartType, setPieChartType] = useState("field");

  // เพิ่ม state สำหรับ AverageBookingsChart filter แยกต่างหาก
  const [bookingsChartFilterPeriod, setBookingsChartFilterPeriod] =
    useState("hourly");
  ...
  const { user } = useAuth();
  // ใน DashboardPage.js
  const { metrics, loading, error } = useRevenueData(
    user,
    selectedMonth,
    selectedYear,
  ...
  console.log(
    "Dashboard Rendering - Selected Month:",
    selectedMonth,
    "Selected Year:",
    selectedYear
  );
